  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 shadow-sm p-4">
      <div className="flex items-center gap-2 mb-1 text-gray-800 font-bold text-sm">
        <Activity className="w-4 h-4 text-[#093eaa]" />{t('Gelecek Projeksiyon — Olası değer (Monte Carlo)')}
      </div>
      <p className="text-[11px] text-gray-400 mb-3">
        {t('Geçmiş getiri/oynaklığın süreceği varsayımıyla olası aralık; kesin tahmin değil. Yön yorumu AI raporunda.')}
      </p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
            {(forecast.portfolioHorizons || []).map(h => (
              <div key={h.months} className="bg-gray-50 rounded-lg p-3 text-center">
                <div className="text-xs font-semibold text-gray-500">{t(h.label)}</div>
                ...
                <div className="text-[11px] text-gray-500 mt-0.5">{t('medyan')} {fmtMoney(h.median)}</div>
                <div className="text-[10px] text-gray-400">{t('aralık')} {fmtMoney(h.p5)} – {fmtMoney(h.p95)}</div>
                <div className="text-[10px] text-rose-500 mt-0.5">{t('kayıp olasılığı')} %{Number(h.probLossPercent).toLocaleString('tr-TR', { maximumFractionDigits: 0 })}</div>
              </div>
            ))}
          </div>
          ...
          {(forecast.assetForecasts || []).length > 0 && (
            <div className="overflow-x-auto">
              <table className="w-full text-xs">
                <thead>
                  <tr className="text-gray-400 border-b border-gray-100">
                    <th className="py-1.5 text-left font-semibold">{t('Varlık')}</th>
                    <th className="py-1.5 text-center font-semibold">{t('Trend')}</th>
                    <th className="py-1.5 text-right font-semibold">{t('1 Ay')}</th>
                    <th className="py-1.5 text-right font-semibold">{t('3 Ay')}</th>
                    <th className="py-1.5 text-right font-semibold">{t('1 Yıl')}</th>
                  </tr>
                </thead>
                <tbody>
              ...
              <p className="text-[10px] text-gray-400 mt-1.5">
                {t('Varlık değerleri medyan (en olası) getiri tahminidir; aşırı geçmiş getiriler ortalamaya dönüş için yumuşatılmıştır. Trend = fiyatın MA20/MA50\'ye göre yönü.')}
              </p>
            </div>
          )}
        </>
