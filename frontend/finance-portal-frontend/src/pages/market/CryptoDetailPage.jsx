              ...
              <div className="px-6 pb-6">
                <div className="h-[520px] relative">
                  ...
                  {!isLoading && displayData.length > 0 && (() => {
                    if (!isComparing) {
                      const lineColor = displayData[displayData.length - 1]?.price >= displayData[0]?.price ? '#10b981' : '#ef4444';
                      return (
                        <ResponsiveContainer width="100%" height="100%">
                          <ComposedChart data={chartDataWithMA} margin={{ top: 5, right: 10, left: 10, bottom: 5 }}>
                            ...
                            <YAxis yAxisId="price" tick={{ fontSize: 10, fill: '#9ca3af' }}
                              tickFormatter={v => fmtPrice(v, currency)} width={90} domain={['auto', 'auto']} />
                            <YAxis yAxisId="volume" orientation="right" tick={{ fontSize: 9, fill: '#d1d5db' }}
                              tickFormatter={v => v >= 1e9 ? `${(v/1e9).toFixed(1)}B` : `${(v/1e6).toFixed(0)}M`} width={50} />
                            <Tooltip content={({ active, payload, label }) => {
                              ...
                              return (
                                <div className="bg-white border border-gray-200 rounded-xl shadow-lg px-4 py-3 text-xs min-w-[180px]">
                                  ...
                                  {price != null && <div className="flex justify-between gap-4"><span className="text-gray-500">Fiyat</span><span className="font-bold text-gray-900">{fmtPrice(price, currency)}</span></div>}
                                  {volume != null && <div className="flex justify-between gap-4 mt-1"><span className="text-gray-500">Hacim</span><span className="font-bold text-gray-600">{parseFloat(volume).toLocaleString('tr-TR', { maximumFractionDigits: 0 })}</span></div>}
                                </div>
                              );
                            }} />
                            <Bar yAxisId="volume" dataKey="volume" fill={lineColor} opacity={0.07} radius={[1,1,0,0]} />
                            <Area yAxisId="price" type="monotone" dataKey="price" stroke={lineColor} strokeWidth={2}
                              fill="url(#cryptoGrad)" dot={false} activeDot={{ r: 4 }} />
                            {/* MA çizgileri */}
                            {MA_OPTIONS.filter(m => activeMA.includes(m.period)).map(m => (
                              <Line key={m.label} yAxisId="price" type="monotone" dataKey={m.label}
                                stroke={m.color} strokeWidth={1.5} dot={false} activeDot={false}
                                strokeDasharray="4 2" connectNulls={false} />
                            ))}
                            <Brush dataKey="date" height={24} stroke="#e5e7eb" fill="#f9fafb" travellerWidth={6}
                              startIndex={Math.max(0, displayData.length - 60)} tickFormatter={() => ''} />
                          </ComposedChart>
                        </ResponsiveContainer>
                      );
                    }
                    return (
                      <ResponsiveContainer width="100%" height="100%">
                        <LineChart data={displayData} margin={{ top: 5, right: 10, left: 10, bottom: 5 }}>
                          ...
                          <Tooltip content={({ active, payload, label }) => {
                            if (!active || !payload?.length) return null;
                            const items = payload.filter(p => !String(p.dataKey).endsWith('_price'));
                            return (
                              <div className="bg-white border border-gray-200 rounded-xl shadow-lg px-4 py-3 text-xs min-w-[200px]">
                                <p className="text-gray-500 font-semibold mb-2 border-b border-gray-100 pb-1">{label}</p>
                                {items.map(p => {
                                  const id = p.dataKey;
                                  const sym = id === coinId ? coin?.symbol?.toUpperCase() : compareCoins.find(c => c.id === id)?.symbol?.toUpperCase() ?? id;
                                  const rawPrice = p.payload?.[`${id}_price`];
                                  const pctVal = p.value;
                                  return (
                                    <div key={id} className="flex items-center justify-between gap-3 mt-1.5">
                                      ...
                                      <span className="text-right">
                                        {rawPrice != null && <span className="font-bold text-gray-900 mr-1.5">{fmtPrice(rawPrice, currency)}</span>}
                                        {pctVal != null && <span className={`font-semibold ${pctVal >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>({pctVal >= 0 ? '+' : ''}{pctVal.toFixed(2)}%)</span>}
                                      </span>
                                    </div>
                                  );
                                })}
                          ...
                          <Line type="monotone" dataKey={coinId} stroke={mainColor} strokeWidth={2} dot={false} activeDot={{ r: 4 }} />
                          {compareCoins.map((c, i) => (
                            <Line key={c.id} type="monotone" dataKey={c.id} stroke={COMPARE_COLORS[i + 1]} strokeWidth={2} dot={false} activeDot={{ r: 4 }} />
                          ))}
                          <Brush dataKey="date" height={24} stroke="#e5e7eb" fill="#f9fafb" travellerWidth={6}
                            startIndex={Math.max(0, displayData.length - 60)} tickFormatter={() => ''} />
                        </LineChart>
                  ...
                  {!isLoading && displayData.length === 0 && (
                    <div className="flex items-center justify-center h-full text-gray-400 text-sm">Grafik verisi yüklenemedi.</div>
                  )}
                </div>
