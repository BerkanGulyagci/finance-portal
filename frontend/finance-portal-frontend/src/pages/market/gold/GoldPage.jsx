...
/* ─── KlineCharts Çizgi Grafiği (Altın için) ─── */
function GoldLineChart({ chartData, currency, isDown }) {
  const chartId = useRef(`kline_gold_line_${Date.now()}`);
  ...
  useEffect(() => {
    ...
    const klineData = chartData
      .map(d => ({
        ...
        open:  parseFloat(d.close) ?? 0,
        high:  parseFloat(d.close) ?? 0,
        low:   parseFloat(d.close) ?? 0,
  ...
  return (
    <div>
      <div id={chartId.current} style={{ width: '100%', height: '280px' }} />
    </div>
  );
}

/* ─── KlineCharts Mum Grafiği (Altın için) ─── */
function GoldCandlestickChart({ chartData, currency }) {
  const chartId = useRef(`kline_gold_candle_${Date.now()}`);
  const chartRef = useRef(null);
  ...
  useEffect(() => {
    ...
    // Candlestick stilini ayarla (varsayılan)
    chart.setStyles({
    ...
    const klineData = chartData
      .map(d => ({
        timestamp: new Date(d.date).getTime(),
        open:  parseFloat(d.open) ?? parseFloat(d.close) ?? 0,
        high:  parseFloat(d.high) ?? parseFloat(d.close) ?? 0,
        low:   parseFloat(d.low) ?? parseFloat(d.close) ?? 0,
        close: parseFloat(d.close) ?? 0,
        volume: d.volume ?? 0,
        turnover: 0,
      }))
...
export default function GoldPage() {
  ...
  const [chartMode, setChartMode] = useState('line'); // 'line' veya 'candle'
  ...
  function handleTabChange(tabKey) {
    ...
    // ONS dışındaki sekmelerde her zaman çizgi grafik göster
    if (tabKey !== 'ons') {
      setChartMode('line');
    }
  }
          ...
          {loadingSpot ? (
            ...
            <>
              ...
              <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
                {/* Grafik modu seçici - sadece ONS sekmesinde göster */}
                {activeTab === 'ons' && (
                  <div className="flex gap-1">
                    {[{ key: 'line', label: 'Çizgi' }, { key: 'candle', label: 'Mum Grafik' }].map(m => (
                      <button
                        key={m.key}
              ...
              <div className="h-72 relative">
                ...
                {chartPoints.length > 0 ? (
                  // Sadece ONS sekmesinde mum grafik seçeneği var, diğerlerinde her zaman çizgi
                  (activeTab === 'ons' && chartMode === 'candle') ? (
                    <GoldCandlestickChart chartData={chartPoints} currency={currency} />
                  ) : (
                    <GoldLineChart chartData={chartPoints} currency={currency} isDown={isDown} />
                  )
                ) : (
