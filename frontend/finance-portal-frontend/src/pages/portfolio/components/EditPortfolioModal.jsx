...
export default function EditPortfolioModal({ portfolio, onClose, onUpdated }) {
  ...
  async function handleSubmit(e) {
    ...
    if (!trimmedName) {
      setError('Portföy adı boş bırakılamaz.');
      return;
    }
    setSaving(true);
    setError('');
    ...
    >
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <h2 className="text-base font-bold text-gray-900">Portföyü Düzenle</h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          ...
          <div className="text-xs text-gray-400 bg-gray-50 rounded-lg px-3 py-2">
            <span className="font-medium text-gray-500">Tür:</span>{' '}
            {portfolio.portfolioType === 'WATCHLIST' ? 'İzleme Listesi' : 'Varlık Portföyü'}
            {'  ·  '}
            <span className="font-medium text-gray-500">Para birimi:</span>{' '}
            {portfolio.currency}
          </div>

          <div className="space-y-1">
            <label className="block text-sm font-medium text-gray-700">
              Portföy Adı <span className="text-rose-500">*</span>
            </label>
            <input
              type="text"
              value={name}
