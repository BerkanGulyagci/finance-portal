  ...
  return (
    <div className={`relative ${className}`} ref={ref}>
      <button
        type="button"
        onClick={() => setOpen(o => !o)}
        className="flex items-center justify-between gap-1 w-full bg-white border border-gray-200 rounded-md px-2.5 py-2 sm:py-1.5 text-sm font-medium text-gray-800 hover:bg-gray-50"
      >
        ...
        <div className={`absolute left-0 mt-1 ${menuWidth} max-h-64 overflow-auto bg-white border border-gray-200 rounded-md shadow-lg z-30`}>
          {options.map(o => (
            <button
              ...
              className={`block w-full text-left px-3 py-2 sm:py-1.5 text-sm hover:bg-gray-50 ${o.value === value ? 'text-[#093eaa] font-bold' : 'text-gray-700'}`}
            >
              {o.label}
            </button>
          ))}
